...
- How to assess regression model assumptions using visualizations and tests.
- How to identify leverage, outliers, and influential points.
- Finding unusual observations in regression models.
...
The Breusch-Pagan Test can not be performed with the default set of packages in `R`. However, the function `bptest` can be loaded from the `lmtest` package to the test.
...
[Skip Ahead.](#unusual-observations) Histograms and Q-Q Plots give a nice visual representation of the residuals distribution, however if we are interested in formal testing, there are a number of options available. A commonly used test is the **Shapiro–Wilk test**, which is implemented in `R`.
...
which we will refer to as the *hat matrix*. The hat matrix is used to project onto the subspace spanned by the columns of $X$. It is also simply known as a projection matrix. Put simply: the hat matrix is a matrix that takes the original $y$ values and adds a hat.
...
Last time we fit an additive regression to the `mtcars` data with `mpg` as the response and `hp` and `am` as predictors. Let's perform some diagnostics on this model.

First, fit the model as we did last lecture.
...
However, we see that there are two points of large leverage.
...
Let's consider the following `big_model`, fit to the `autompg` dataset. It uses `mpg` as the response, and considered many interaction terms between the predictors `disp`, `hp`, and `domestic`.
